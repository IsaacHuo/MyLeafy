...
extension PerformanceRefactorTests {
    ...
    func testContinuousZoomGeometryFillsSevenAndThreeDayViewports() {
        let week = TimetableZoomGeometry(
            progress: 0,
            horizontalOffset: 0,
            viewportWidth: 700,
            anchorOrdinal: 12,
        ...
        XCTAssertEqual(week.laneWidth(dayOfWeek: 1), 100, accuracy: 0.001)
        XCTAssertEqual(week.contentTranslation, -700, accuracy: 0.001)

        let threeDay = TimetableZoomGeometry(
            progress: 1,
            horizontalOffset: 0,
            viewportWidth: 700,
            anchorOrdinal: 12,
        ...
        XCTAssertEqual(threeDay.laneWidth(dayOfWeek: 6), 700 / 3, accuracy: 0.001)
        XCTAssertEqual(
            threeDay.laneFrame(ordinal: 12, dayOfWeek: 6).midX,
            350,
            accuracy: 0.001
        )
    }

    func testContinuousZoomGeometryExpandsHiddenWeekendFromZeroWidth() {
        let week = TimetableZoomGeometry(
            progress: 0,
            horizontalOffset: 0,
            viewportWidth: 500,
        ...
        XCTAssertEqual(week.laneWidth(dayOfWeek: 1), 100, accuracy: 0.001)
        XCTAssertEqual(week.laneWidth(dayOfWeek: 6), 0, accuracy: 0.001)
        ...
        XCTAssertEqual(threeDay.laneWidth(dayOfWeek: 6), 500 / 3, accuracy: 0.001)
        XCTAssertEqual(
            threeDay.laneFrame(ordinal: 12, dayOfWeek: 6).midX,
            250,
            accuracy: 0.001
    ...
    func testContinuousZoomGeometryIsExactlyReversible() {
        for step in 0...20 {
            let progress = CGFloat(step) / 20
            let forward = TimetableZoomGeometry(
                progress: progress,
                horizontalOffset: 0,
                viewportWidth: 390,
                anchorOrdinal: 8,
            ...
            let reverse = TimetableZoomGeometry(
                progress: progress,
                horizontalOffset: 0,
                viewportWidth: 390,
                anchorOrdinal: 8,
            ...
            XCTAssertEqual(
                forward.laneFrame(ordinal: 8, dayOfWeek: 2),
                reverse.laneFrame(ordinal: 8, dayOfWeek: 2)
            )
        }
    }
    ...
    func testContinuousViewportCentersSaturdayAndPagesByThreeDays() throws {
        ...
        let controller = TimetableContinuousViewportController(
            ...
            centerDate: monday,
            rangeStart: try XCTUnwrap(formatter.date(from: "2026-01-01")),
            rangeEnd: try XCTUnwrap(formatter.date(from: "2027-12-31")),
            calendar: calendar
        )

        controller.beginMagnification(centersToday: true, today: saturday)
        controller.updateMagnification(1.5)
        controller.endMagnification(magnification: 1.5, velocity: 0, reducesMotion: true)
        XCTAssertEqual(formatter.string(from: controller.centerDate), "2026-08-22")
        XCTAssertEqual(
            controller.threeDayDates.map { formatter.string(from: $0) },
        ...
        XCTAssertEqual(formatter.string(from: controller.centerDate), "2026-08-25")
    }

    @MainActor
    func testContinuousViewportUsesTuesdayForHistoricalWeekAndReturnsCurrentPageWeek() throws {
        let (calendar, formatter) = continuousZoomCalendar()
        let monday = try XCTUnwrap(formatter.date(from: "2026-12-28"))
        let controller = TimetableContinuousViewportController(
            weekStartDate: monday,
            centerDate: monday,
            rangeStart: try XCTUnwrap(formatter.date(from: "2026-01-01")),
        ...
        XCTAssertEqual(formatter.string(from: controller.centerDate), "2027-01-01")

        controller.beginMagnification(centersToday: false)
        controller.updateMagnification(1 / 1.5)
        controller.endMagnification(magnification: 1 / 1.5, velocity: 0, reducesMotion: true)
        XCTAssertEqual(formatter.string(from: controller.weekStartDate), "2026-12-28")
        XCTAssertEqual(controller.zoomProgress, 0, accuracy: 0.001)
    }

    @MainActor
    func testContinuousViewportVelocityAndGestureUpdatesDoNotRebuildWindow() throws {
        let (calendar, formatter) = continuousZoomCalendar()
        let monday = try XCTUnwrap(formatter.date(from: "2026-08-17"))
        let controller = TimetableContinuousViewportController(
            weekStartDate: monday,
        ...
        controller.beginMagnification(centersToday: false)
        for step in 0..<100 {
            controller.updateMagnification(1 + CGFloat(step) / 1_000)
        }
        XCTAssertEqual(controller.windowRevision, revision)
        XCTAssertEqual(controller.gestureUpdateCount, 100)
        controller.endMagnification(magnification: 1.05, velocity: 3, reducesMotion: true)
        XCTAssertEqual(controller.zoomProgress, 1, accuracy: 0.001)
        XCTAssertEqual(controller.windowRevision, revision)
    }

    private func continuousZoomCalendar() -> (Calendar, DateFormatter) {
