...
- iOS 课表通过网格顶部下拉刷新，周视图、三日视图和未铺满屏幕的课表均使用原生刷新控件；左上角快捷菜单不再提供重新同步。下拉只更新最新可拉取学期的课表及其关联提醒、Widget 和已发布共享快照，保留现有步骤与结果反馈；Demo 仅更新示例课程，通用学校和游客不启用教务刷新。下拉负偏移同步时间轴但不保存为浏览位置；空课表刷新期间保持网格挂载，身份恢复由原有流程接管。
...
- 空闲教室通过学校 `jsjy_query2` POST 表单获取矩阵，节次使用两位编码。按教室查询一次获取全天数据，按节次查询从全天矩阵筛选；按返回 `tdvalue` 展开组合节次。缺失教室、未知符号、缺失节次和网络失败不得推断空闲/占用或写为成功缓存。日期必须对应已配置学期，不能夹到第一/最后一周。
