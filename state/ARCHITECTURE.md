...
- 本科自动恢复使用 Keychain 中与当前身份匹配的账号密码和同一 URLSession 获取、提交验证码；验证码字母按学校规则统一为小写，原图、四倍 Lanczos 放大图、四倍放大灰度增强图三路必须得到相同的 `[a-z0-9]{4}`，且每路置信度不低于 0.85 时，最多自动提交一次。结果冲突、格式或置信度不合格、识别、登录或重试失败均转人工；研究生端不做自动 OCR。
