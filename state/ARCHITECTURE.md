...
- 课表周视图可通过连续双指形变切换为三日模式；初始页围绕手势中心日期，左右每次移动三个不重叠自然日。三日模式始终索引七天内容且不受“隐藏周末”影响，捏合后返回当前三日页中间日期所属周。周/三日模式共用 `TimetableViewportState` 的日期锚点、页面与纵向偏移，并在目标容器完成布局后才移除缩放过渡层。
