...
Deno.test("critical multi-write actions declare transaction RPC boundaries", () => {
  ...
  for (const action of ["createCommunityBanner", "upsertTeacher", "upsertCourse", "upsertDish"]) {
...
Deno.test("high-risk additions require operator or super-admin", () => {
  ...
  assert(source.includes('publishCommunityBanner: defineAction("publishCommunityBanner", publishCommunityBanner, { domain: "admin", permission: "operator", mutating: true })'));
});

Deno.test("community banners validate targets and private image uploads", () => {
  for (const route of [
    "timetable",
    "community",
    "schedule_reports",
    ...
    assert(source.includes(`"${route}"`), `missing allowed app route ${route}`);
  }
