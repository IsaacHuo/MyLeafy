create index if not exists campus_ai_usage_events_reserved_created_idx
  on private.campus_ai_usage_events (created_at)
  where status = 'reserved';

create or replace function private.expire_stale_campus_ai_reservations(
...
begin
  perform private.expire_stale_campus_ai_reservations(p_now);
...
begin
  ...
  if (snapshot->>'daily_remaining')::integer <= 0 then
    return jsonb_build_object('allowed', false, 'error', 'daily_quota_exhausted', 'quota', snapshot);
  end if;

  if selected_plan_source = 'subscription'
     and coalesce((snapshot->>'period_remaining')::integer, 0) <= 0 then
    return jsonb_build_object('allowed', false, 'error', 'period_quota_exhausted', 'quota', snapshot);
  end if;

  insert into private.campus_ai_usage_events (
    request_uuid, auth_user_id, app_transaction_id, campus_id,
    provider, model, plan_source, status, quota_units
  ) values (
    p_request_uuid, p_auth_user_id, normalized_app_transaction_id,
    coalesce(nullif(btrim(p_campus_id), ''), 'unknown'),
    'deepseek', 'deepseek-v4-flash', selected_plan_source, 'reserved', 1
  );
...
revoke all on function private.expire_stale_campus_ai_reservations(timestamptz)
  from public, anon, authenticated;
grant execute on function private.expire_stale_campus_ai_reservations(timestamptz)
  to service_role;
