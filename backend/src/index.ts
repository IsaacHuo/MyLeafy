...
import { catalog, rate, ownRatings, suggestCatalog } from './catalog';
import { ownedInvites } from './sharing';
import { commentThreads } from './comment-threads';
import { activityPosts, activityComments, profileDetail, profileStats, polls, getTerms } from './community-reads';
import { announcements, announcementRead, notificationSettings, dismissNotification, banner, postgraduateSources, submitFeedback } from './notices';
...
app.get('/v1/catalog/:kind/ratings',async c=>c.json(await ownRatings(c.env,await actor(c.env,c.req.raw),c.req.param('kind'))));
app.post('/v1/catalog/suggestions',async c=>c.json(await suggestCatalog(c.env,await actor(c.env,c.req.raw),await readJSON(c.req.raw))));
...
app.get('/v1/timetables/invites',async c=>c.json(await ownedInvites(c.env,await actor(c.env,c.req.raw),new URL(c.req.url))));
...
app.get('/v1/timetables/members',async c=>c.json(await shareMembers(c.env,await actor(c.env,c.req.raw),['incoming','viewing'].includes(c.req.query('direction')??''))));
...
app.get('/v1/profiles/:id',async c=>c.json(await profileDetail(c.env,await actor(c.env,c.req.raw),c.req.param('id'))));
app.post('/v1/profiles/stats',async c=>c.json(await profileStats(c.env,await actor(c.env,c.req.raw),await readJSON(c.req.raw))));
app.get('/v1/community/activity/posts',async c=>c.json(await activityPosts(c.env,await actor(c.env,c.req.raw),new URL(c.req.url))));
app.get('/v1/community/activity/comments',async c=>c.json(await activityComments(c.env,await actor(c.env,c.req.raw),new URL(c.req.url))));
app.get('/v1/community/polls',async c=>c.json(await polls(c.env,await actor(c.env,c.req.raw),new URL(c.req.url))));
app.get('/v1/community/terms',async c=>c.json(await getTerms(c.env,await actor(c.env,c.req.raw))));
...
app.get('/v1/community/posts/:id/comment-threads',async c=>c.json(await commentThreads(c.env,await actor(c.env,c.req.raw),c.req.param('id'),new URL(c.req.url))));
...
app.post('/v1/community/comments/:id/toggle-like',async c=>c.json(await toggleCommentLike(c.env,await actor(c.env,c.req.raw),c.req.param('id'),await readJSON(c.req.raw))));
app.post('/v1/community/posts/:id/toggle-like',async c=>{const who=await actor(c.env,c.req.raw);await setPostReaction(c.env,who,c.req.param('id'),'like','toggle');return c.json(await postDetail(c.env,who,c.req.param('id')));});
app.post('/v1/community/posts/:id/toggle-favorite',async c=>{const who=await actor(c.env,c.req.raw);await setPostReaction(c.env,who,c.req.param('id'),'favorite','toggle');return c.json(await postDetail(c.env,who,c.req.param('id')));});
app.get('/v1/community/posts/:id/pending',async c=>c.json(await pendingPost(c.env,await actor(c.env,c.req.raw),c.req.param('id'))));
app.post('/v1/community/posts/:id/abort',async c=>c.json(await pendingPost(c.env,await actor(c.env,c.req.raw),c.req.param('id'),true)));
...
app.put('/v1/community/blocks/:id',async c=>c.json(await setBlock(c.env,await actor(c.env,c.req.raw),c.req.param('id'),true,(await readJSON(c.req.raw)).reason)));
...
app.get('/v1/notifications/settings',async c=>c.json(await notificationSettings(c.env,await actor(c.env,c.req.raw))));
app.put('/v1/notifications/settings',async c=>c.json(await notificationSettings(c.env,await actor(c.env,c.req.raw),await readJSON(c.req.raw))));
app.post('/v1/notifications/:id/dismiss',async c=>c.json(await dismissNotification(c.env,await actor(c.env,c.req.raw),c.req.param('id'))));
app.get('/v1/announcements',async c=>c.json(await announcements(c.env,await actor(c.env,c.req.raw),new URL(c.req.url))));
app.post('/v1/announcements/:id/read',async c=>c.json(await announcementRead(c.env,await actor(c.env,c.req.raw),c.req.param('id'))));
app.post('/v1/announcements/:id/dismiss',async c=>c.json(await announcementRead(c.env,await actor(c.env,c.req.raw),c.req.param('id'),true)));
app.get('/v1/community/banner',async c=>c.json(await banner(c.env,await actor(c.env,c.req.raw))));
app.get('/v1/postgraduate-sources',async c=>c.json(await postgraduateSources(c.env,await actor(c.env,c.req.raw),new URL(c.req.url))));
app.post('/v1/feedback',async c=>c.json(await submitFeedback(c.env,c.req.raw,await readJSON(c.req.raw))));
