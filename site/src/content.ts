...
export const privacySections: Array<{
...
}> = [
  {
    ...
    items: [
      ...
      "MyLeafy AI：当你使用免费或订阅请求时，你的问题以及经你批准的设备内信息会通过 MyLeafy AI 服务发送给 DeepSeek。为提供与保护服务，我们会处理 Apple 提供的交易编号、已验证的订阅信息、使用次数、次数重置时间与必要的请求诊断信息。这些记录不会保存你的完整问题。"
    ]
  },
  {
    title: "处理目的",
  ...
  },
  {
    ...
    items: [
      ...
      "DeepSeek 处理 MyLeafy AI 问题与经你批准的信息，用于生成回答。如你使用自己的 DeepSeek 密钥，请求会直接从设备发送，密钥只保存在系统安全存储中。"
    ]
  },
  {
    id: "retention",
