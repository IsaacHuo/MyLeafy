...
- 支持关闭、照片、纯色和效果；关闭不会删除已经选择的照片。
- 照片支持显示模式、可见度、模糊、遮罩、课程卡片浓度和 6 款固定滤镜。
- 纯色使用系统颜色选择器；效果提供柔彩、波纹、流光，以及森林、晨曦、夜空配色。
- 课程配色随照片提取色、纯色或效果配色自动生成。
- 滤镜与效果选择网格使用固定帧，只有顶部预览和课表当前动态效果持续更新。
...
- 校园页不再做首页式大卡片堆叠，而是通过一级功能切换展示对应工具。
...
- 顶部使用实时课表预览，下面以分段选择器切换关闭、照片、纯色和效果。
- 照片模式通过照片选择器配置背景，并以网格选择原图或 6 款固定滤镜。
- 纯色模式使用系统 `ColorPicker`，效果模式以固定帧网格选择效果和配色。
