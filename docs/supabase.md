...
生产或 TestFlight 环境使用自定义 SMTP。当前非北林 / 自定义学校账号首次注册先用邮箱验证码确认邮箱，再由 App 为已验证会话设置密码；之后使用邮箱和密码登录。建议接入阿里云 DirectMail / 邮件推送，避免 Resend 在国内邮箱上的明显延迟。Cloudflare Email Routing 只用于 `support@myleafy.space` 这类收信转发，不负责 Auth 验证码投递。
...
7. 在 iOS App 的 `其他学校 / 自定义` 入口测试首次注册邮箱验证码，以及后续邮箱密码登录。
8. 先登录教务，再进入 `社区`。
