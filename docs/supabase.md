...
初始化是不可接管的状态机：相同 Auth 和教务身份可恢复原账号；Auth 已绑定另一教务身份时拒绝；教务身份已归属另一 Auth 时进入邮箱恢复；只有尚未绑定的教务身份可被首次占用。并发首次占用由数据库事务和唯一约束裁决。
...
- `profile_auth_links` 对每个 profile 只保留一个有效 Auth 主体。
...
北京林业大学社区账号可绑定已验证邮箱。该邮箱用于换设备、重装或社区会话丢失后的账号恢复，不参与日常登录，也不改变北林登录方式；北林仍使用学号、教务密码和教务验证码。恢复成功后必须再次核对本机当前教务身份与 profile 绑定一致。注意：
