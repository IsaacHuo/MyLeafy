...
- 新建社区 Banner 使用“创建并发布”：保存成功后立即上线，并原子归档同一学校此前发布的 Banner；自动发布失败时保留草稿，运营人员可从列表重试发布。Banner 图片通过后台申请的一次性 signed upload URL 由浏览器直传私有 Storage，创建请求只提交对象路径；服务端接管前仍会验证学校、管理员、格式、大小与尺寸。
...
Feed 预览应调用与 iOS 客户端一致的服务端排序规则。浏览器不维护独立排序逻辑。
...
`npm run build` 已包含 TypeScript 检查。`npm run test:e2e` 使用 Chromium 与 WebKit，保留给重大后台交互调整和发布前验证，不作为每次站点提交的日常 CI。
