...
- 邀请限时且单次接受。
- 同一用户可以同时生成多个邀请码；已到期的邀请码由数据库定时任务清理。
...
- 所有权优先关联 profile 或稳定业务主体；客户端提交的任意 user ID 不作为所有权依据。
...
- 当前客户端使用带 request ID 的 `create_community_post_v4`、`create_community_comment_v2` 重载，并在 PostgREST 命名参数中显式发送可空字段。服务端按 community actor + request ID 返回首次创建结果，同时保留顶层评论省略回复目标、帖子省略空分类的精确兼容重载。已发布客户端使用的旧参数签名继续可执行，重复的固定 post UUID 返回已有帖子。该兼容边界不恢复已移除的独立 publish RPC，也不改变媒体校验链路。
...
- 可提前创建下学期配置；学校开放课表后由运营后台人工设为 active，不必等待正式开学日。
...
- 激活前必须从学校真实页面确认所有参数，不能根据年份猜测；active 表示当前拉取目标，`semester_start_date` 独立承担正式周次计算。
...
必须通过经过权限控制的后台 action 原子切换 active 配置；数据库不再按日期自动切换或回退，且每个校园始终最多一个 active 配置。
...
不要把获取的个人联系方式、未经授权的评价文本或敏感学校字段写入目录。
...
```bash
mapfile -t entrypoints < <(find supabase/functions -name '*.ts' -type f | sort)
deno check "${entrypoints[@]}"
deno test --allow-env --allow-read --allow-sys supabase/functions
```
