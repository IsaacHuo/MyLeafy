...
产品范围见[项目总览](../product/overview.md)，页面规则见[App 产品设计](../design/app-design.md)，后端与管理端分别见 [Supabase 接入](supabase.md)和[运营后台](admin-console.md)。

## 1. 架构目标

MyLeafy 同时面对三类性质不同的系统：不稳定的学校网页、强调本地体验的 iOS 客户端，以及需要严格授权的云端社区和运营业务。架构重点是隔离变化，分层数量由实际职责决定。

- 将学校 HTML、Cookie 和页面跳转限制在教务基础设施层。
- 页面依赖业务协议和投影模型，不直接拼装网络请求。
...
应用层只负责组合，不应承载教务页面解析或复杂业务计算。纯视觉细节属于表现层和主题组件：选中衬底、卡片表面、Liquid Glass、图片边距、淡化过渡和星级居中不进入 Domain。
...
- `TimetableWebViewBootstrapper`：课表 HTTP 路径失败后的浏览器兼容方案。
...
公开根 Tab 顺序为 `课表 / 社区 / 日迹 / 校园 / 我的`，默认选中课表。日迹顶部直接提供 `随记 / 日程 / 推送`。随记元数据、Markdown 源文、图片、附件和音频均使用校园身份作用域的本地存储；个人日程列表支持无限自然年，当前学年内的日期投射到课表，学年外日期保留为个人倒计时。学校课程、考试和校历不进入随记或个人日程列表。社区不可用时直接移除对应入口，不显示无法使用的空页面。
...
flowchart TB
    ...
    Request["03 · 获取并识别页面<br/>URLSession 主链路 · WKWebView 特定兼容路径"]
...
- `+Discover.swift`：考试、学校教学、教室等学业请求。
...
`WKWebView` 仅用于课表的特定兼容路径，不承担通用网页自动化。学校页面发生变化时，应优先修复请求识别和解析器，避免扩大 WebView 依赖。
...
课表的单学年容器按绝对日期组织，范围从秋季学期开始至下一学年开始前一天；暑假最后一周在学年边界结束，下一学年通过学年或日期选择进入。学校课表响应仍按学期保存为 20 周数据集，课程发生只来自学校响应，未获取的未来学期保持为空。学期结束日期和寒暑假范围来自语义校历事件。
...
性能测量使用相同设备、构建配置、数据和交互，各采集三次并比较中位数。`LeafyPerformanceSignposter` 只记录课表快照、评分加载和社区 Feed projection 等不含用户文本的区间。只有中位数改善至少 10%、峰值内存回退不超过 5% 且无新增 app-owned 泄漏时，才对外宣称提速。
...
状态责任按层划分：Domain 只测试业务状态和纯规则；Application 测试用例状态、缓存交付和错误分类；Presentation 测试导航选择、Loading/Content/Empty/Error/Unauthenticated/Partial 的映射，以及系统控件的可达性。颜色、表面、间距、Liquid Glass、淡化动画和星级居中属于视觉验收，不写入领域测试。

课表测试覆盖绝对日期学年边界、暑假末周、学年选择、20 周学期容器、缓存命中和未获取未来学期。日迹测试覆盖三段顶部导航、Tag 筛选与“全部随记”、个人日程自然年范围，以及学校课程/考试/校历与本地随记的隔离。

| 范围 | 主要测试 |
|---|---|
| Domain / Application | XCTest：纯规则、绝对日期学年边界、20 周学期容器、投影、路由、数据迁移、用例状态与错误分类 |
...
| Presentation / UI | XCTest 或 SwiftUI UI 测试：根 Tab 顺序、日迹三段导航、状态映射、可访问性和关键布局验收 |
