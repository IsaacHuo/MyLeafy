...
flowchart LR
    ...
    subgraph Tasks["五个公开核心任务区"]
        ...
        Nav --> Schedule["日程<br/>随记 · 个人日程<br/>回顾 · 标签 · 统计"]
        Nav --> Campus["校园<br/>学校教学 · 教室<br/>学习空间 · 扩展工具"]
    ...
    class App,Nav root;
    class Timetable,Community,Schedule,Campus,Profile area;
    class Shared shared;
```
...
“年度视图”入口位于“日程 → 年度视图”，并复用课表的学期和日历配置。该页面提供以下年度信息：

- 月/周范围浏览。
- 课程、考试、倒计时和提醒的聚合。
- 指定日期详情。
- 从年度视图返回日程或继续进入相关时间工具。
...
采用单列随记卡片流和底部快速输入器，侧栏承载个人日程、每日回顾、标签、统计、日程推送、年度视图和回收站。随记与学校课程、考试、校历保持独立数据源；学校考试和校历仍从“学校教学”进入。
