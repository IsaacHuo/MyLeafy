...
- 当前技术实现与数据流：[当前架构](../../state/ARCHITECTURE.md)
...
`日迹`进入后直接显示顶部的 `随记 / 日程 / 推送` 分区。侧栏“记录”分组把 `记录日迹` 放在 `每日回顾` 上方。记录日迹按自然年展示每月随记数和记录天数，并提供近 30 天热力、连续记录、星期/时段习惯、常用标签和里程碑；点按热力图日期可进入当天随记。统计图片由本机生成，只含聚合统计，不含随记正文或标签名，通过系统分享且不上传。个人日程列表支持无限自然年；当前学年内的日期可投射到课表，学年外的日期保留为个人倒计时。升级后新建的日程还会以同一条记录投影到随记流，按创建时间排序并支持搜索，但不进入随记统计、回顾、标签、导出和回收站。随记输入器支持聚焦展开，达到四个视觉行时可放大或缩小编辑；加号菜单不再提供新建文章入口，历史文章仍可查看和编辑。学校课程、考试和校历不进入随记或个人日程列表。
