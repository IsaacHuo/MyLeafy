...
当前 `main` 不包含 MyLeafy AI 的 iOS 页面、模型访问和购买实现，公开信息架构中也没有 AI 入口。App 只保留旧数据模型与 Keychain 清理路径；AI 相关后端资产继续存在以兼容冻结分支。若未来明确恢复该能力，应以 `codex/leafy-ai` 的实现为起点，重新完成产品、隐私、订阅和真实设备验收后，再把当时有效的交互规范写回本文。
