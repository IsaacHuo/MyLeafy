...
- 验证码加载失败时在表单下方显示微说明。
- 登录失败用系统 alert。
- 已登录用户主动刷新教务数据时显示连接、验证码刷新/识别轮次和登录验证，不展示验证码内容或置信度。无法访问校园网时只弹出连接提示，用户关闭并连接网络后再次操作；不提前展示人工验证 sheet，也不自动监听网络重试。本科最多显示三轮自动验证，第三轮仍失败才展示人工验证 sheet；保存凭据可用时默认只显示验证码，并提供“修改账号或密码”的渐进入口。
