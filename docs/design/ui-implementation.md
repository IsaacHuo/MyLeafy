...
根结构按 `课表 / 社区 / 日迹 / 校园 / 我的` 排列：

- 课表：SF Symbol `calendar`。
- 社区：SF Symbol `person.2`。
- 日迹：SF Symbol `note.text`。
...
- 社区 Tab 使用系统 badge。
- 底部栏使用系统 TabView 的原生选中状态，不叠加透明度伪淡入或自定义浮层。

日迹进入后，顶部使用 `随记 / 日程 / 推送` 三个分区。外层导航保留 Liquid Glass；分区切换不添加白色选中衬底。侧栏“记录”分组按 `记录日迹 / 每日回顾 / 标签 / 导出随记 / 回收站` 排列，记录日迹位于每日回顾上方。

随记卡片使用系统白色表面。Tag 使用白字主题色胶囊；筛选后显示当前 Tag，并提供“全部随记”返回全部内容。记录日迹使用自然年选择器、月度柱状统计、近 30 天可点按热力图、星期和时段图表、常用标签与里程碑卡片；点按日期打开当天随记。右上角在本机渲染只含聚合数字的统计图片，经系统分享 sheet 分享，图片不含正文或标签名，也不上传。输入器支持聚焦展开，达到四个视觉行时可放大或缩小编辑；加号菜单不提供新建文章入口，历史文章仍可查看和编辑。图片预览保留左侧间距，预览内容不贴齐屏幕边缘。

升级后新建的个人日程以同一条记录投影为随记卡片，使用真实创建时间参与默认倒序和全文搜索；点击卡片进入原日程编辑器。旧日程不补写创建时间，也不进入随记流。

校园侧栏切换时保留内容淡化过渡，淡化只表达切换过程，不改变数据状态或加载结果。
...
- 验证码加载失败时在表单下方显示微说明。
- 登录失败用系统 alert。
- 已登录用户主动刷新教务数据时只显示“正在连接教务系统…”或原页面加载态，不展示 OCR 术语。无法访问校园网时只弹出连接提示，用户关闭并连接网络后再次操作；不提前展示人工验证 sheet，也不自动监听网络重试。Session 失效且三路一致性识别不可靠时才展示人工验证 sheet；保存凭据可用时默认只显示验证码，并提供“修改账号或密码”的渐进入口。
- 用户主动教务操作使用居中 Leafy 进度卡：只包含系统 `ProgressView`、SF Symbols、文本和现有卡片样式，不模拟 Alert 或承载操作按钮。卡片保留已完成、当前和失败步骤；人工验证码继续使用系统 sheet，同步结果继续使用系统 alert，查询成功直接显示结果。
...
- 页面根背景可以显示用户选择的照片或纯色。
- 课表主体优先使用横向连续周视图网格。
- 单学年容器按绝对日期组织，从秋季学期开始至下一学年开始前一天；暑假最后一周停在学年边界。下一学年通过学年或日期选择进入，学校学期响应仍按每学期 20 周处理。
...
- 周态和三日态不切换 View；同一组 21 日日期列通过 `zoomProgress` 插值 lane 宽度与整体平移，Header、网格、当前时间线和卡片严格共用几何。
- 本周双指放大后以今天居中并显示昨天、今天、明天；其他周以周二居中显示周一至周三。左右吸附分页每次移动三个不重叠自然日，捏合后返回当前三日页中间日期所属周。
- 三日模式忽略“隐藏周末”的列过滤，共用周课表的课程、考试、日程投影和纵向节次滚动。
- 手势过程以 1.5 倍舒适映射直接驱动几何，不添加动画或改变字号；松手后结合 progress 与 velocity，使用 120Hz 可中断低回弹 Spring 完成或回弹。
- 隐藏周末时七日数据仍常驻，周态周末 lane 为零宽，放大后连续展开。减弱动态效果仍保持手势跟随，松手后直接吸附端点，并提供系统无障碍缩放操作。
...
- 未明确设置显示模式时默认完整显示；更换照片会保持启用并立即刷新课表根背景层。
...
- 紧凑卡和完整卡都直接显示全部选项、百分比与线性进度，不以是否已投票作为可见条件。
...
- 自习安排内的空闲教室为内部工具；图书馆座位预约和校园热力图也在自习安排中。专注记录归入学习空间。
...
- 2026–2027 第一学期长跑周期截止 1 月 15 日，整周跳过中秋、国庆和元旦涉及的第 3、4、5、17 周。
...
- 评分详情 sheet 内提交或更新星级评分，五颗星在评分区域水平居中。
- 缺失项提交按钮放在标题区域右侧。
