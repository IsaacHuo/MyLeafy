...
- 双指放大以手势中心日期进入连续三列，左右吸附分页每次移动三个不重叠自然日；捏合后返回当前三日页中间日期所属周。
- 三日模式忽略“隐藏周末”的列过滤，共用周课表的课程、考试、日程投影和纵向节次滚动。
- 手势过程直接插值列宽、间距、字级与透明度，松手后使用可中断弹簧完成或回弹；目标页布局与偏移就绪后才撤下过渡层。
- 减弱动态效果开启时跳过列宽形变，仅对齐目标页并使用短透明度切换，同时提供系统无障碍缩放操作。
