...
- `leafy/Features/`：Auth、Timetable、Discover、Community、Profile 等用户可见功能。
...
系统 `TabView` 负责承载页面；iOS 26 以上使用系统 tab bar 和 Liquid Glass 外观，iOS 17 继续使用自定义浮动 Tab Bar。跨页面跳转由 `AppNavigationCoordinator` 协调，例如从课表跳到学业页的自习室查询。
...
当前 Supabase 能力包括资料完善、邮箱验证绑定、帖子流、全局/分类置顶、发帖、图片上传、评论、点赞、删除、我的发帖、我的点赞、我的评论、通知、站内公告、意见反馈、老师星级评分、共享课表和 Leafy AI 托管额度。
