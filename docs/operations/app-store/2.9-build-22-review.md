...
3. Guideline 2.1(b)：MyLeafy AI 周订阅在 Sandbox 购买时显示交易验证失败。

本次发布策略是暂时隐藏 MyLeafy AI 和购买入口，以四个公开根 Tab 重新提交；同时修复两个与 AI 订阅无关的底层崩溃。
...
审核截图显示 MyLeafy AI 周订阅页面返回：

> 同步 MyLeafy AI 额度失败：订阅交易验证失败，请稍后重试。
...
MyLeafy AI 和订阅实现继续保留在源码中，但本次审核版本没有公开入口：

- 根导航固定为 `课表 / 社区 / 校园 / 我的`；
- iOS 26 原生 `Tab` 和 iOS 17–25 `tabItem` 两条路径都不展示 MyLeafy AI；
- `RootTab.leafy` 类型继续保留，便于后续恢复；
- 任何旧状态或内部状态尝试选择 `.leafy`，都会重定向到 `.timetable`；
- 用户无法从公开 UI 到达 MyLeafy AI 或购买页面。
...
- [ ] 删除本版本元数据、宣传文本和审核截图中关于 MyLeafy AI 或周订阅可购买的描述。
- [ ] 隐私政策链接保持可访问。
...
- MyLeafy AI、商品 ID、SwiftData schema、Supabase RPC 和 AI 服务代码均未删除。
- 未降低或绕过订阅验签安全策略。
...
```text
MyLeafy 2.9 (build 22) temporarily removes the public MyLeafy AI entry point and all in-app purchase access. The subscription product is not included in this submission.
...
- MyLeafy AI and its in-app purchase flow have been removed from all public entry points, and the subscription is not included in this submission.
